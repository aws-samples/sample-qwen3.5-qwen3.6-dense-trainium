...
#
# Modified from the libtorch-neuronx-lite 2.11.0.1.0.1284 source shipped
# with vllm-neuron 0.24.0.1.1.0, for Qwen3.5/3.6 dense enablement; see
# overrides/README.md for the delta.
import logging
import operator
...
class AliasingOutputRewritePass(FXPass):
    ...
    def _is_nki_call_with_mutations(self, node: Node) -> bool:
        ...
        """
        from libtorch_neuronx_lite.nki.nki_hop import NKIKernelWrapper
    ...
    def _is_data_attr_op(self, node: Node) -> bool:
        """Check if a node is a ``.data`` access (Tensor.data attribute read).

        ``tensor.data`` traces to ``torch._C._autograd._get_data_attr``, whose
        output aliases the same storage as its input. It is common in the
        DeltaNet state-buffer write pattern ``buffer.data.copy_(new_state)``:
        the ``copy_`` mutates the ``.data`` view, so without treating
        ``_get_data_attr`` as an alias of its source the mutation cannot be
        traced back to the buffer placeholder and no ``io_map`` entry is
        emitted — the state update is then silently dropped across the
        separately-compiled prefill/decode graphs.
        """
        if node.op != "call_function":
            return False
        target_name = getattr(node.target, "__name__", str(node.target))
        return target_name == "_get_data_attr" or target_name.endswith(
            "_get_data_attr"
        )
    ...
    def _is_aliasing_op(self, node: Node) -> bool:
        ...
        Returns:
            True if the node is a slice, method-aliasing, ATen-aliasing, or
            ``.data`` attribute-access op.
        """
        return (
            self._is_slice_operation(node)
            or self._is_method_aliasing_op(node)
            or self._is_aten_aliasing_op(node)
            or self._is_data_attr_op(node)
        )
