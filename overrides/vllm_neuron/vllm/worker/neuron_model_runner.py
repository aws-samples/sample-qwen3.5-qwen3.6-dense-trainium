...
#
# Modified from the vllm-neuron 0.24.0.1.1.0 source (tag v0.24.0.1.1.0) for
# Qwen3.5/3.6 dense enablement; see overrides/README.md for the delta.
"""
...
class NeuronModelRunner(KVConnectorModelRunnerMixin, NeuronECConnectorModelRunnerMixin):
    ...
    def __init__(
        ...
        # DeltaNet (linear-attention) side-channel state slots. Hybrid models
        # like Qwen3.5/3.6 carry recurrent/conv state in per-layer buffers
        # instead of the paged KV cache; those buffers are addressed by a
        # REQUEST-STABLE slot index (not the batch position, which vLLM's
        # continuous batching reshuffles). The runner owns the req_id -> slot
        # mapping and threads it to the model via attn_metadata["state_slots"]
        # each step, mirroring the block-table lifecycle: assigned at first
        # schedule, freed on finish/abort/preemption.
        # `_state_slot_capacity`: None = not yet resolved from the model,
        # -1 = model has no DeltaNet state buffers (feature disabled).
        self._state_slot_capacity: int | None = None
        self._state_slot_by_req: dict[str, int] = {}
    ...
    def _reset_state(self) -> None:
        ...
        # Clear request cache
        self.requests.clear()
        self._state_slot_by_req.clear()

        logger.debug("Model runner state reset successfully")

    # ── DeltaNet side-channel state slots ────────────────────────────────
    # See docs/implementation-notes.md §3 (state-slot fix); model side:
    # src/qwen3_6/model_bf16.py. Hybrid models keep linear-attention
    # recurrent/conv state in per-layer buffers of shape [capacity + 1, ...];
    # row `capacity` is a scratch slot for padded batch rows. The runner
    # assigns each request a stable row for its whole lifetime and passes the
    # per-step row indices to the model via attn_metadata["state_slots"].

    def _resolve_state_slot_capacity(self, model) -> None:
        """Detect DeltaNet state buffers on the model (once, at load)."""
        capacity = -1
        for module in model.modules():
            scratch = getattr(module, "state_scratch_slot", None)
            if scratch is not None:
                capacity = int(scratch)
                break
        self._state_slot_capacity = capacity
        if capacity >= 0:
            logger.info(
                "DeltaNet state-slot allocation enabled: %s slots + scratch",
                capacity,
            )

    def _state_slots_enabled(self) -> bool:
        return (
            self._state_slot_capacity is not None and self._state_slot_capacity >= 0
        )

    def _acquire_state_slot(self, req_id: str) -> int:
        """Return the request's slot, allocating on first use."""
        slot = self._state_slot_by_req.get(req_id)
        if slot is None:
            in_use = set(self._state_slot_by_req.values())
            slot = next(
                (s for s in range(self._state_slot_capacity) if s not in in_use),
                None,
            )
            if slot is None:
                # Capacity mismatch between the model's state buffers
                # (sized at model init from
                # max(neuron_config.num_seqs_buckets)) and the scheduler's
                # concurrent-request cap. This should be impossible when the
                # sequence buckets are derived from max_num_seqs; fail loudly
                # rather than corrupt state.
                raise RuntimeError(
                    f"DeltaNet state slots exhausted: {len(in_use)} live "
                    f"requests >= capacity {self._state_slot_capacity} "
                    f"(new req_id={req_id})"
                )
            self._state_slot_by_req[req_id] = slot
        return slot

    def _release_state_slot(self, req_id: str) -> None:
        self._state_slot_by_req.pop(req_id, None)

    def _release_state_slots_for(self, scheduler_output: "SchedulerOutput") -> None:
        """Free the state slots of requests leaving the batch this step.

        Called from `execute_model` immediately before `_update_states`, which
        is a verbatim upstream copy and must stay unmodified (see the NEURON
        MAINTAINER NOTE on that method).

        Finished requests are gone for good. Preempted requests lose their KV
        blocks and re-prefill from scratch on resume, so their DeltaNet state
        row is dead too — release it (a fresh slot is acquired when the resume
        is scheduled). NOTE: merely-unscheduled requests (e.g. decodes parked
        during another request's prefill step) keep their slot — their state
        must survive across steps.
        """
        if not self._state_slots_enabled():
            return
    ...
    def _build_state_slots_tensor(self, padded_num_reqs: int) -> torch.Tensor:
        """Per-step [padded_num_reqs] tensor: batch position -> buffer row.

        Padded positions (no request) point at the scratch slot so their
        writes never touch a live request's state.
        """
        scratch = self._state_slot_capacity
        slots = torch.full((padded_num_reqs,), scratch, dtype=torch.long)
        for pos, req_id in enumerate(self.input_batch.req_ids):
            if pos >= padded_num_reqs:
                break
            slots[pos] = self._acquire_state_slot(req_id)
        return slots.to(self.device)
    ...
    def load_model(self) -> None:
        ...
        # Resolve DeltaNet state-slot capacity from the plain module BEFORE
        # torch.compile / tensor-capture wrapping (attribute delegation on the
        # wrappers is not guaranteed).
        self._resolve_state_slot_capacity(self.model)
    ...
    def _build_attention_metadata(
        ...
        # Request-stable DeltaNet state-slot indices for this step (hybrid
        # linear-attention models only). One entry per padded batch row.
        # Prefill carries one real sequence, so only row 0 is a live request
        # slot; padded_num_reqs may still exceed 1 under DP padding, and those
        # extra rows point at the scratch slot. The model consumes
        # state_slots[:1] for the prefill write.
        state_slots = (
            self._build_state_slots_tensor(padded_num_reqs)
            if self._state_slots_enabled()
            else None
        )
        ...
        for kv_cache_group_id, kv_cache_group_spec in enumerate(
            ...
            if state_slots is not None:
                attn_metadata_i["state_slots"] = state_slots
    ...
    def _build_warmup_attention_metadata(
        ...
        for _, kv_cache_group_spec in enumerate(self.kv_cache_config.kv_cache_groups):
            ...
            if swa_kv_pos_offset is not None:
                attn_metadata_i["swa_kv_pos_offset"] = swa_kv_pos_offset
            if self._state_slots_enabled():
                # Identity slots for warmup — same [num_reqs] shape as the
                # runtime tensor so the compiled graph matches.
                attn_metadata_i["state_slots"] = torch.arange(
    ...
    def execute_model(
        ...
        with record_function_or_nullcontext("neuron_model_runner: update_states"):
            # Release DeltaNet state slots for requests leaving the batch
            # BEFORE _update_states drops their cached state. Kept out of
            # _update_states itself, which is a verbatim upstream copy.
            self._release_state_slots_for(scheduler_output)
    ...
    def execute_dummy_batch(self) -> None:
        ...
        if self._state_slots_enabled():
            # Unlike warmup, the dummy batch runs mid-serving on an idle DP
            # rank that may still hold live (parked) requests' DeltaNet state.
            # The warmup helper's identity slots would alias rows 0..N-1, so
            # point every row at the scratch slot instead — dummy writes must
            # never touch a live request's state. Same [num_reqs] shape as
            # the identity tensor, so the compiled graph still matches.
            scratch_slots = torch.full(
                (batch_size,),
                self._state_slot_capacity,
                dtype=torch.long,
                device=self.device,
            )
            for attn_metadata_i in decode_kwargs["attn_metadata"].values():
                attn_metadata_i["state_slots"] = scratch_slots
