...
# ===========================================================================
# This file is derived from aws-neuron/neuronx-distributed-inference
...
# Upstream PR #152 validation environment (historical, not a requirement):
# trn2.48xlarge, NKI 0.3.0 / Neuron SDK 2.29+, TP=4, BF16
# This repository validates it on trn2.3xlarge, vllm-neuron 0.24.0.1.1.0
# (see the README's compatibility matrix).
#
# Keep the algorithm and math aligned with the upstream kernel, and document
# local modifications explicitly. Wrap it from
# qwen3_6.model_bf16.Qwen3_6DeltaNetAttention. When the NKI HOP ABI in
# libtorch-neuronx-lite shifts, fix the wrapper rather than this file.
#
# LOCAL MODIFICATIONS:
# - Comments and phase headings corrected to match the code (text only).
# - Dead-code cleanup: removed unused NumPy mask/identity helper functions;
#   callers provide the masks as registered buffers from model_bf16.py.
# - BF16 I/O: q/k/v and `output` follow the input dtype (bf16 or fp32); ALL
#   internal math (cumsum, exp decays, triangular solve, state, every matmul
#   operand) stays fp32 — q/k/v are upcast to fp32 working copies right after
#   load and the fp32 result is downcast once before the output store. g/beta
#   and final_state remain fp32.
#
# This is NOT a departure from the reference semantics — it mirrors the HF
# Qwen3.5 gated-delta-rule op exactly (transformers
# models/qwen3_5/modeling_qwen3_5.py, torch_chunk_gated_delta_rule):
#     initial_dtype = query.dtype                              # bf16
#     query, key, value, beta, g = [x....to(torch.float32) ...]  # fp32 compute
#     core_attn_out = core_attn_out....to(initial_dtype)       # bf16 output
# with the recurrent state kept fp32 (cf. config mamba_ssm_dtype: float32).
# PR #152 originally hard-typed the I/O fp32; passing bf16 straight from the
# projection (as HF does) is the faithful contract. With fp32 inputs the casts
# are no-ops, so the original fp32 behavior is preserved.
#
# NOTE: bf16 *matmul operands* were also tried (fp32 PSUM preserved) and
# measured to give NO speedup — the 128x128 matmuls are not the bottleneck;
# the serial fp32 triangular solve / cumsum / exp chain dominates. Reverted.
# Validated vs fp64 CPU reference, passed.
#
...
Comparison baseline (not vendored here): PR #152's per-chunk kernel at
``contrib/models/Qwen3.5-4B/src/nki_kernels/nki_deltanet_chunked.py``, pinned
to PR head commit ``c1795b0000d84c2aa2978b35adf4e6bcc69bc670``:
https://github.com/aws-neuron/neuronx-distributed-inference/blob/c1795b0000d84c2aa2978b35adf4e6bcc69bc670/contrib/models/Qwen3.5-4B/src/nki_kernels/nki_deltanet_chunked.py

Key optimizations over that per-chunk baseline:
  ...
  5. Uses tensor_scalar for row-wise scaling: each (P,1) operand broadcasts
     across the free dimension without materializing a (P,F) operand
  6. Uses nc_transpose for 128x128 transposes; on Trn2 this is a Tensor Engine
     identity-matrix matmul, replacing the baseline's explicit equivalent
  7. Forms decay as exp(cumsum(g)_i - cumsum(g)_j), never as split
     exp(cumsum(g)_i) * exp(-cumsum(g)_j)

Shape constraints:
  k_dim = v_dim = 128 = P_MAX exactly.
...
Mathematical framework:
  Per-chunk blocked triangular solve for intra-chunk correction:
    ...
    N = inv(I - A), computed by two independent 64x64 forward substitutions
        (for N11 and N22) plus one 64->128 lower-block merge
    value_corr = N @ v_beta
...
def deltanet_fused_chunked_fwd(
    query: nl.ndarray,  # (S, 128) bf16|fp32 — l2-normed and scaled
    key: nl.ndarray,  # (S, 128) bf16|fp32 — l2-normed
    value: nl.ndarray,  # (S, 128) bf16|fp32
    ...
    Input requirements:
      ...
      - query/key/value may be bf16 or fp32; they are upcast to fp32 internally
        so all math runs in fp32 regardless. g_in/beta_in must be fp32.

    Returns:
        output:      (S, 128) — same dtype as `query` (bf16 or fp32)
    ...
    for i_chunk in nl.sequential_range(num_chunks):
        ...
        # ---- Load chunk data from HBM (in input dtype: bf16 or fp32) ----
        # q/k/v are DMA'd in their native dtype to keep HBM traffic low, then
        # upcast to fp32 working copies (q_c/k_c/v_c) so every downstream op —
        # transposes, matmuls, decays — runs in fp32 exactly as the verbatim
        # fp32 kernel did. When the input is already fp32 these tensor_copies
        # are fp32->fp32 no-ops.
        q_in_c = nl.ndarray((P_MAX, dim), dtype=query.dtype, buffer=nl.sbuf)
        nisa.dma_copy(
            dst=q_in_c,
            src=query[chunk_start : chunk_start + CHUNK_SIZE, 0:dim],
        )
        q_c = nl.ndarray((P_MAX, dim), dtype=nl.float32, buffer=nl.sbuf)
        nisa.tensor_copy(dst=q_c, src=q_in_c)

        k_in_c = nl.ndarray((P_MAX, dim), dtype=key.dtype, buffer=nl.sbuf)
        nisa.dma_copy(
            dst=k_in_c,
            src=key[chunk_start : chunk_start + CHUNK_SIZE, 0:dim],
        )
        k_c = nl.ndarray((P_MAX, dim), dtype=nl.float32, buffer=nl.sbuf)
        nisa.tensor_copy(dst=k_c, src=k_in_c)

        v_in_c = nl.ndarray((P_MAX, dim), dtype=value.dtype, buffer=nl.sbuf)
        nisa.dma_copy(
            dst=v_in_c,
            src=value[chunk_start : chunk_start + CHUNK_SIZE, 0:dim],
        )
        v_c = nl.ndarray((P_MAX, dim), dtype=nl.float32, buffer=nl.sbuf)
        nisa.tensor_copy(dst=v_c, src=v_in_c)
        ...
        # Strict-lower decay for the triangular solve: exp(gc[i] - gc[j])
        # where i > j.
        ...
        # ============================================================
        # Phase 2a: Intra-chunk attention scores
        # attn_intra = (q @ k^T) * decay_diag   (decay_diag already carries the
        # lower-with-diagonal mask, so this is a single multiply)
        # ============================================================
        ...
        # ============================================================
        # Phase 2b: inter-chunk propagation — v_prime = k_cumdecay @ state
        ...
        # Downcast the fp32 result to the output dtype (bf16 or fp32) before the
        # store. tensor_copy performs the dtype conversion; fp32->fp32 is a no-op.
        chunk_out_cast = nl.ndarray((P_MAX, dim), dtype=output.dtype, buffer=nl.sbuf)
